...
final class RollKeepTests: XCTestCase {
	func test5k3Pool() {
		let pool = RollAndKeepRoll(roll: 5, keep: 3)
	...
	func test10k10Pool() {
		let pool = RollAndKeepRoll(roll: 10, keep: 10)
	...
	func test11k4Pool() {
		let pool = RollAndKeepRoll(roll: 11, keep: 4)
		XCTAssertEqual(pool.pool.dice.count, 10)
	...
	func test14k4Pool() {
		let pool = RollAndKeepRoll(roll: 14, keep: 4)
		XCTAssertEqual(pool.pool.dice.count, 10)
	...
	func test13k9Pool() {
		let pool = RollAndKeepRoll(roll: 13, keep: 9)
		XCTAssertEqual(pool.pool.dice.count, 10)
		XCTAssertEqual(pool.diceActuallyKept, 10)
	...
	func test10k12Pool() {
		let pool = RollAndKeepRoll(roll: 10, keep: 12)
		XCTAssertEqual(pool.pool.dice.count, 10)
		XCTAssertEqual(pool.diceActuallyKept, 10)
	...
	func test16k12Pool() {
		let pool = RollAndKeepRoll(roll: 16, keep: 12)
		XCTAssertEqual(pool.pool.dice.count, 10)
		XCTAssertEqual(pool.diceActuallyKept, 10)
